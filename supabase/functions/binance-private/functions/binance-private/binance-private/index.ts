 ...
 * - Credentials are stored encrypted in Supabase Vault.
 * - The authenticated user's verified ID is used to locate only their secret.
...
function normalizeSafeError(error: unknown) {
  ...
  return allowed.has(message)
    ? message
    : 'BINANCE_PRIVATE_REQUEST_FAILED';
}
...
function validateReadOnlyPermissions(permission: any) {
  if (!permission || typeof permission !== 'object') {
    return {
      valid: false,
      readOnly: false,
      reason: 'BINANCE_PERMISSION_RESPONSE_INVALID',
    };
  }
  ...
  const forbidden = [
    ...
    [
      ...
      permission.enableSpotAndMarginTrading,
    ],
    ...
    [
      ...
      permission.enablePortfolioMarginTrading,
    ],
  ...
  const readOnly =
    permission.enableReading === true &&
    enabledForbidden.length === 0;
  ...
  return {
    ...
    permission: {
      ...
      enableInternalTransfer:
        permission.enableInternalTransfer === true,
      permitsUniversalTransfer:
        permission.permitsUniversalTransfer === true,
      enableSpotAndMarginTrading:
        permission.enableSpotAndMarginTrading === true,
      enableMargin:
        permission.enableMargin === true,
      enableFutures:
        permission.enableFutures === true,
      enableVanillaOptions:
        permission.enableVanillaOptions === true,
      enablePortfolioMarginTrading:
        permission.enablePortfolioMarginTrading === true,
      enableFixApiTrade:
        permission.enableFixApiTrade === true,
      ipRestrict:
        permission.ipRestrict === true,
      enableFixReadOnly:
        permission.enableFixReadOnly === true,
    },
  };
}
...
function buildFillsParams(
  ...
  const startTime =
    positiveIntegerOrUndefined(params?.startTime);

  const endTime =
    positiveIntegerOrUndefined(params?.endTime);

  const fromId =
    positiveIntegerOrUndefined(params?.fromId);
  ...
  const limit =
    ...
    params?.limit === ''
      ? undefined
      : positiveIntegerOrUndefined(params.limit);
...
async function loadBinanceCredentials(
  ...
  if (error) {
    console.error(
      'binance-private credentials lookup failed:',
      error.message,
    );
...
async function handle(
  ...
  /* ==========================================================
     IMPORTANTE:
     Las credenciales solo se leen mediante supabaseAdmin,
  ...
  const {
    apiKey,
    apiSecret,
  } = await loadBinanceCredentials(
    ctx.supabaseAdmin,
    userId,
  );

  /* ==========================================================
     PRIMERA LLAMADA PRIVADA:
     comprobar que la API Key realmente es READ-ONLY.
     ========================================================== */
  ...
  /* ==========================================================
     RESTO DE OPERACIONES READ-ONLY
     ========================================================== */
  ...
  fetch: withSupabase(
    ...
    async (
      ...
      try {
        ...
        console.error(
          'binance-private error:',
          normalizeSafeError(error),
        );
